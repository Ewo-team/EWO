...
ControleAcces('utilisateur',1);
	...
	for($inc=1 ; $inc<=$nb_perso ; $inc++){
		if($_SESSION['persos']['race'][$inc]==4){
			$carte_ok = 1;
			}
		if($_SESSION['persos']['race'][$inc]==-1){
			$trich = 1;
			}
		if(isset($_SESSION['persos']['carte'][$inc]) && $_SESSION['persos']['carte'][$inc]==2  && $_SESSION['persos']['race'][$inc] && $_SESSION['persos']['race'][$inc]!=4){
			$carte_en = 1;
			}
			
		if($_SESSION['persos']['faction']['id'][$inc]  && $_SESSION['persos']['race'][$inc] && $_SESSION['persos']['race'][$inc]!=4){
...
if(($carte_ok || $carte_en || $carte_fac) && !$trich){
	...
	for($inc=1 ; $inc<=$nb_perso ; $inc++){
		...
		if(isset($_SESSION['persos']['carte'][$inc]) && $_SESSION['persos']['carte'][$inc]==2 && $_SESSION['persos']['race'][$inc]!=4){
			$carte_en = 1;
			...
			$val_min = max(-100,$_SESSION['persos']['pos_y'][$inc]-$perception);
			$val_max = min(0,$_SESSION['persos']['pos_y'][$inc]+$perception);
	...
	for($inc=1 ; $inc<=$n_fac ; $inc++){
		...
		$rchch_x_tmp='';
			...
			$val_min = max(-100,$perso_fac_id['pos_y'][$inc]-$perception);
			$val_max = min(0,$perso_fac_id['pos_y'][$inc]+$perception);
...
if (isset($_SESSION['utilisateur']['carte_enfer']['race'][$grace][$ggrade])){
	if($_SESSION['utilisateur']['carte_enfer']['race'][$grace][$ggrade]+90<$time){
		$time_up = true;
		$_SESSION['utilisateur']['carte_enfer']['race'][$grace][$ggrade] = $time;
		}else $time_up = false ;
	} else {
		$_SESSION['utilisateur']['carte_enfer']['race'][$grace][$ggrade] = $time;
		$time_up = true;
...
if (isset($_SESSION['utilisateur']['carte_enfer']['porte'])){
	if($_SESSION['utilisateur']['carte_enfer']['porte']+90<$time){
		$time_up = true;
		$_SESSION['utilisateur']['carte_enfer']['porte'] = $time;
		}else $time_up = false ;
	} else {
		$_SESSION['utilisateur']['carte_enfer']['porte'] = $time;
		$time_up = true;
...
if (isset($_SESSION['utilisateur']['carte_enfer']['bouclier'])){
	if($_SESSION['utilisateur']['carte_enfer']['bouclier']+90<$time){
		$time_up = true;
		$_SESSION['utilisateur']['carte_enfer']['bouclier'] = $time;
		}else $time_up = false ;
	} else {
		$_SESSION['utilisateur']['carte_enfer']['bouclier'] = $time;
		$time_up = true;
...
if (isset($_SESSION['utilisateur']['carte_enfer']['grille'])){
	if($_SESSION['utilisateur']['carte_enfer']['grille']+90<$time){
		$time_up = true;
		$_SESSION['utilisateur']['carte_enfer']['grille'] = $time;
		}else $time_up = false ;
	} else {
		$_SESSION['utilisateur']['carte_enfer']['grille'] = $time;
		$time_up = true;
...
$time_up = $time_up || $carte_en;
	...
	//$carte = imagecreatefromjpeg("../images/cartes/plan/enfer.jpg");
	$coeff=10;
	$maxi_x=400;
	$maxi_y=400;
...
if(isset($_GET['race']) && !preg_match('#[a-z]#i',$_GET['race']) && isset($_GET['grade']) && !preg_match('#[a-z]#i',$_GET['grade'])){
	...
	imagepng($maxi_carte,"./enfer_R".$_GET['race']."G".$_GET['grade'].".png");
	imagecolortransparent($mini_carte, $fond);
	imagepng($mini_carte,"./Genfer_R".$_GET['race']."G".$_GET['grade'].".png");
	}else{
		$maxi_carte = imagecreatefrompng("./enfer_R".$_GET['race']."G".$_GET['grade'].".png");
		$mini_carte = imagecreatefrompng("./Genfer_R".$_GET['race']."G".$_GET['grade'].".png");
		}
}
...
if(isset($_GET['porte']) && $_GET['porte']==1){
	...
	imagecopyresized($maxi_carte, $mini_carte, 0, 0, 0, 0, imagesx($maxi_carte), imagesy($maxi_carte), imagesx($mini_carte), imagesy($mini_carte));
	imagecolortransparent($maxi_carte, $fond);
	imagepng($maxi_carte,"./enfer_porte.png");
	imagecolortransparent($mini_carte, $fond);
	imagepng($mini_carte,"./Genfer_porte.png");
	}else{
		$maxi_carte = imagecreatefrompng("./enfer_porte.png");
		$mini_carte = imagecreatefrompng("./Genfer_porte.png");
		}
}
...
if(isset($_GET['bouclier']) && $_GET['bouclier']==1){
	...
	imagecopyresized($maxi_carte, $mini_carte, 0, 0, 0, 0, imagesx($maxi_carte), imagesy($maxi_carte), imagesx($mini_carte), imagesy($mini_carte));
	imagecolortransparent($maxi_carte, $fond);
	imagepng($maxi_carte,"./enfer_bouclier.png");
	imagecolortransparent($mini_carte, $fond);
	imagepng($mini_carte,"./Genfer_bouclier.png");
	}else{
		$maxi_carte = imagecreatefrompng("./enfer_bouclier.png");
		$mini_carte = imagecreatefrompng("./Genfer_bouclier.png");
		}
}
...
if(isset($_GET['grille']) && $_GET['grille']==1){
	if($time_up){
		...
		for($inci=1;$inci<=4;$inci++){
			...
			imagepng($maxi_carte,"./grille_plan_en.png");
			}
		for($inci=1;$inci<=4;$inci++){
			...
			$maxi_carte = imagecreatefrompng("./grille_plan_en.png");
			}
