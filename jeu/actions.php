...
if(!isset($_SESSION['gain_xp']['att']))
	...
	/*// GESTION DES SORTS (touche + esquive)
	 if (isset($event_sort) && isset($_SESSION['esquive']['nb']) && $_SESSION['esquive']['nb']>0){
		$liste_Victime = "";
		$liste_Esquive = "";

		if($reussite){
		//recuperation de la liste de cible touché et des cible qui ont esquivé
		for ($inci=1;$inci<=$_SESSION['esquive']['nb'];$inci++) {
		$l_mat_cible = $_SESSION['esquive']['mat'][$inci];
		if ($_SESSION['esquive']['type'][$inci]=="persos"){ //la cible est un perso on lui ajoute un event esquive
		if ($_SESSION['esquive']['val'][$inci]) {
		//La cible a esquivé le sort
		$event_Esquive_Magique = new EventsManager($l_mat_cible,"esquive_magique");
		$event_Esquive_Magique->addEvent('attaquant',$current_perso_id);
		$event_Esquive_Magique->addEvent('reussite',$reussite);
		$event_Esquive_Magique->addEvent('sort',$idAction);
		$event_Esquive_Magique->addEvent('exp',$_SESSION['gain_xp']['def'][$l_mat_cible]);
		$event_Esquive_Magique->commitEvent();
		}else{ // La cible n'a pas esquivé le sort
		//Recherche de la victime dans la liste des morts
		$vict_morte=0;
		for($incj=1; $incj<=$_SESSION['mort']['nb'];$incj++){
		if($l_mat_cible==$_SESSION['mort']['id'][$incj]){
		$vict_morte=1;
		}
		}
		if(!$vict_morte){
		$event_vict_sort = new EventsManager($l_mat_cible,"sort");
		//On se sert de cet emplacement pour récupérer l'id de l'attaquant
		$event_vict_sort->addEvent('attaquant',$current_perso_id);
		$event_vict_sort->addEvent('sort',$idAction);
		$event_vict_sort->addEvent('exp',$_SESSION['gain_xp']['def'][$l_mat_cible]);
		$event_vict_sort->commitEvent();
		}
		}
		//listing des autres cibles du sort.
		}
		}
		}
		$event_sort->addEvent('attaquant',$current_perso_id);
		$event_sort->addEvent('reussite',$reussite);
		$event_sort->addEvent('l_perso_vict',$l_perso_vict);
		$event_sort->addEvent('l_os_vict',$l_os_vict);
		$event_sort->addEvent('l_oc_vict',$l_oc_vict);
		$event_sort->addEvent('l_p_vict',$l_p_vict);
		$event_sort->addEvent('l_b_vict',$l_b_vict);
		$event_sort->addEvent('l_perso_mort',$l_perso_mort);
		$event_sort->addEvent('l_os_det',$l_os_det);
		$event_sort->addEvent('l_oc_det',$l_os_det);
		$event_sort->addEvent('l_p_det',$l_os_det);
		$event_sort->addEvent('l_b_det',$l_os_det);
		$event_sort->addEvent('sort',$idAction);
		$event_sort->addEvent('exp_att',$_SESSION['gain_xp']['att']);
		$event_sort->commitEvent();
		}

		//Annonce des meurtres et destruction
	 if($_SESSION['mort']['nb']>0){
	 $event_meurtre = new EventsManager($current_perso_id,"meurtre");
	 $event_meurtre->addEvent('nombre_mort',$_SESSION['mort']['nb']);
	 $event_meurtre->addEvent('liste_victime',$l_perso_mort);
	 $event_meurtre->commitEvent();

	 for($inci=1;$inci<=$_SESSION['mort']['nb'];$inci++){
	 //$eventScheme = array('exp','att','def','attaquant', 'type_mort', 'type_attaquant', 'liste_effet');
	 $event_mort = new EventsManager($_SESSION['mort']['id'][$inci],"mort");
	 $event_mort->addEvent('attaquant',$current_perso_id);
	 $l_mat_cible=$_SESSION['mort']['id'][$inci];
	 $event_mort->addEvent('exp',$_SESSION['gain_xp']['def'][$l_mat_cible]);
	 $event_mort->addEvent('type_attaquant','persos');
	 $event_mort->addEvent('type_mort',$type_action);
	 if($type_action=='attaque'){
	 $event_mort->addEvent('att',$jet_att);
	 $event_mort->addEvent('def',$jet_def);
	 $event_mort->addEvent('liste_effet',$_SESSION['score']['degats']);
	 }
	 $event_mort->commitEvent();
	 }

	 }

	 if($_SESSION['destruction']['nb']>0){
		$event_destruction = new EventsManager($current_perso_id,"destruction");
		$event_destruction->addEvent('nombre_objet',$_SESSION['destruction']['nb']);
		//$eventScheme = array('nombre_objet','l_os_det', 'l_oc_det', 'l_p_det', 'l_b_det');
		$event_destruction->addEvent('l_os_det',$l_os_det);
		$event_destruction->addEvent('l_oc_det',$l_oc_det);
		$event_destruction->addEvent('l_p_det',$l_p_det);
		$event_destruction->addEvent('l_b_det',$l_b_det);
		$event_destruction->commitEvent();
		}*/
