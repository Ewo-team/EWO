...
if ($is_log) {
	...
	$menu['utilisateur'][] = array('url' => $root_url.'/compte/', 'nom' => 'Mon compte');
        $menu['utilisateur'][] = array('url' => $root_url.'/carte/', 'nom' => 'Carte du Monde');
        $menu['utilisateur'][] = array('url' => $root_url.'/annuaire/', 'nom' => 'Annuaire');
	...
	$menu['utilisateur'][] = array('url' => $root_url.'/event/liste_events.php', 'nom' => 'Mes &eacute;v&eacute;nements');
        $menu['utilisateur'][] = array('url' => $root_url.'/affiliation/', 'nom' => 'Affiliations '.$nom_aff);
...
// Fin "Savoir si l'utilisateur peut encore créer des persos."
	...
	//$menu['utilisateur'][] = array('url' => $root_url.'/affiliation/liste_persos.php', 'nom' => 'Affiliation personnages'.$nom_aff);
	$menu['utilisateur'][] = array('url' => $root_url.'/legion/', 'nom' => 'Légions personnages'.$nom_fac);

	$menu['persos'][] = array('url' => $root_url.'/persos/liste_persos.php', 'nom' => 'Pages de jeu');

	if($creerperso) {
		$menu['persos'][] = array('url' => $root_url.'/inscription/creation_perso.php', 'nom' => '<i>Créer un personnage</i>');
	}
	...
	for($inci=1; $inci<=$nbperso; $inci++) {
        ...
        $menu['forum'][] = array('url' => $root_url.'/forum/', 'nom' => 'Forum');
	...
	/*if($is_admin) {
        ...
        if($is_anim) {
		$menu['admin'][] = array('url' => $root_url.'/admin/', 'nom' => 'Powa Pannel');
		//$menu['admin'][] = array('url' => $root_url.'/admin/', 'nom' => 'Administration');
                if($is_at) {
	...
	if($is_at && !$is_anim) {
        ...
        $menu['login'][] = array('url' => $root_url.'/session.php', 'nom' => 'Déconnexion', 'class' => 'current');
    ...
    $menu['login'][] = array('url' => $root_url.'/connexion/', 'nom' => 'Connexion');
}
